...
func spawn_hearts(global_position: Vector2, amount: int,zindex: int = 0 , spacing: float = 20.0, heart_position_relativ_to_parent: float = -20 ):
	...
	for i in range(amount):
		...
		heart.position = global_position + Vector2(i * spacing, heart_position_relativ_to_parent) 
		heart.z_index = 18
